  ...
  actions: {
    ...
    async receiveLoginSignal({commit, state}) {
      ...
      try {
        ...
        const budget_response = await axios.get(`${api_address}user/${state.user.user_id}/user_budget`);
        console.log(`Monthly budget : ${JSON.stringify(budget_response.data)}`);
        commit("setMonthlyBudget", { monthlyBudget: budget_response.data });        
        const expenses_response = await axios.get(`${api_address}user/${state.user.user_id}/expenses`);        
        console.log(`Expenses list: ${JSON.stringify(expenses_response.data)}`);        
        commit("setExpensesList", { expensesList: expenses_response.data.expenses });        
        const eco_goals_response = await axios.get(`${api_address}user/${state.user.user_id}/eco_goals`);        
        console.log(`Eco Goals list: ${JSON.stringify(eco_goals_response.data)}`);        
        if (eco_goals_response.data.eco_goals) {
          commit("setEcoGoalsList", { ecoGoalsList: eco_goals_response.data.eco_goals });
        }
        const eco_actions_response = await axios.get(`${api_address}user/${state.user.user_id}/eco_actions`);        
